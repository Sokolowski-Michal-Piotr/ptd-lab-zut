...
Version:0.3.13*
* - means it's modification of such version
****************************************************************************/
using namespace std;
#include "matplotpp.h"
...
//
void MatPlot::color(std::string c) {
    if (mode) {
        cl->Color = c;
    }
}
...
float MatPlot::ctx2(double x) {
    ...
    throw std::exception("xscale unpredicted");
}
float MatPlot::cty2(double y) {
    ...
    throw std::exception("yscale unpredicted");
}
// axes coordination
...
void MatPlot::display_axes_2d() {
    ...
    if (ca->XAxisLocation == "top") { x_axis_location = 1; }
    else { x_axis_location = -1; }
    if (ca->YAxisLocation == "right") { y_axis_location = 1; }
    else { y_axis_location = -1; }

    int char_w = 6, char_h = 12;
    //
    //float offset = 0.01;
    //
    //float wFactor = window_w / 1920.f;
    //float hFactor = window_h / 1080.f;
    //float whFactor = wFactor < hFactor ? wFactor : hFactor;
    //float offset = this->offset * whFactor;
    //
    int num_char = 4;
    ...
    if (ca->Box) {
        ...
        if (ca->TickLabel) {
            for (int i = 0; i < ca->XTick.size(); ++i) {
                ...
                //
                sprintf(ctmp, "%.1f", ca->XTick[i]);
                num_char = strlen(ctmp);
                //
...
vector<float> MatPlot::colormap(string c, float t) {
    ...
    throw std::exception("colormap unpredicted");
}
